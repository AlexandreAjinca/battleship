...
class Game extends Component {
	...
	handleClick(i) {
		...
		if (isPlacementPhase) {
			...
			if (selectedBoat) {
				...
				if (this.isAligned(coords, i) && this.isAdjacent(coords, i)) {
	...
	render() {
		...
		const board = this.state.placementPhase
			? selectedPlayer.boardBoats
			: selectedPlayer.boardStrike;
		...
		const instruction = this.state.selectedBoat ? (
			<div>
				{"Placez ce bateau "} <br />
				<button onClick={() => this.selectBoat(this.state.indexSelectedBoat)}>
					Réinitialiser bateau
				</button>
			</div>
		) : this.state.placementPhase ? (
